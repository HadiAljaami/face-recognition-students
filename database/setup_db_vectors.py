...
def create_roles_and_permissions_tables():
    try:
        ...
        CREATE TABLE IF NOT EXISTS user_roles (
            ...
            role_id INTEGER NOT NULL REFERENCES roles(id) ON DELETE CASCADE,
            created_at TIMESTAMP DEFAULT NOW(),  # ⬅️ الإضافة الوحيدة
            UNIQUE(user_id, role_id)
        );
        """

        execute_query(query_roles)
        print("✅ Table 'roles' created.")

        execute_query(query_permissions)
        print("✅ Table 'permissions' created.")

        execute_query(query_role_permissions)
        print("✅ Table 'role_permissions' created.")

        execute_query(query_user_roles)
        print("✅ Table 'user_roles' created.")

    except Exception as e:
        print(f"❌ Error creating roles/permissions tables: {e}")
